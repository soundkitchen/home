...
" Indent ActionScript anonymous classes correctly.
...
setlocal indentexpr=GetActionScriptIndent()

let b:undo_indent = "set cin< cino< indentkeys< indentexpr<"

" Only define the function once.
if exists("*GetActionScriptIndent")
  finish
endif

function! SkipActionScriptBlanksAndComments(startline)
...
function GetActionScriptIndent()

  " Action is just like C; use the built-in C indenting and then correct a few
  " specific cases.
  ...
  let lnum = SkipActionScriptBlanksAndComments(v:lnum - 1)
  let prev = lnum
  while prev > 1
    let next_prev = SkipActionScriptBlanksAndComments(prev - 1)
  ...
  if getline(v:lnum) =~ '^\s*}\s*\(//.*\|/\*.*\)\=$'
    ...
    if lnum < v:lnum
      while lnum > 1
        let next_lnum = SkipActionScriptBlanksAndComments(lnum - 1)
